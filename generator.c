 ...
 *
 * compile: cc -O -o generator generator.c -lm
...
#define randm(x)    (lrand48x() % (unsigned long long) (x))
...
long long generator(int n, long long *pp, long long *ww, int type, long long r, int v, int tests)
{
  int i;
  long long wsum, psum, c, w, p, r1, r2, k1, k2; 
  FILE *out;
...
void showitems(int n, long long *pp, long long *ww, long long c, int name)
{
  int i;
  long long ps, ws;
  FILE *out;
  ...
  fprintf(out,"%llu ", c);
  for (i = 0; i < n; i++) {
    fprintf(out, "%llu ", ww[i]);
  }
  for (i = 0; i < n; i++) {
    fprintf(out, "%llu ", pp[i]);
  }
  //fprintf(out,"%lld\n", c);
  fclose(out);
}
...
void main(int argc, char *argv[])
{
  ...
  if (argc == 7) {
    ...
    S = atoi(argv[5]);
    name = atoi(argv[6]);
    //printf("generator2 %d %llu %d %d %d %d\n", n, r, type, i, S, name);
  } else {
    //printf("generator2\n");
    printf("n = ");
    ok = scanf("%d", &n);
  ...
  pp = (long long *) malloc(n * sizeof(long long));
  ww = (long long *) malloc(n * sizeof(long long));
  c = generator(n, pp, ww, type, r, i, S);
  showitems(n, pp, ww, c, name);
  free(pp);
  free(ww);
}
